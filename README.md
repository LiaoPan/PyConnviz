...
examples. The first two use real MSDL connectivity projected to fsaverage; the
Plotly and circle previews use deterministic simulated connectivity.

<table>
  <thead>
    <tr>
      <th>Matplotlib surface: three publication views</th>
      <th>Native Nilearn surface: six pial views</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td width="50%"><img src="docs/images/surface-matplotlib.png" width="100%" alt="PyConnviz Matplotlib surface connectome with left, right, and dorsal views"></td>
      <td width="50%"><img src="docs/images/surface-nilearn.png" width="100%" alt="PyConnviz native Nilearn surface connectome with lateral, medial, and dorsal views"></td>
    </tr>
  </tbody>
</table>
...
<table>
  <thead>
    <tr>
      <th>Plotly surface: interactive-backend preview</th>
      <th>Nilearn glass brain</th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
result = plot_connectome(
    ...
    engine="matplotlib",
    max_edges=4,
    title="PyConnviz quickstart",
    output="quickstart_surface.png",
    show=False,
)
print(result.output_files)
print(result.prepared.edges)  # exact edges visible in the figure
```

The complete runnable version is `examples/01_numpy_surface.py`.
...
surface = plot_connectome(prepared, geometry, output="alpha_surface.png")
...
`cortex_alpha` controls cortical opacity in all three surface engines and must be a
finite value from `0` to `1`. The style defaults are `0.40` for `paper`, `0.35`
for `soft`, and `0.55` for `dark`. Static surface networks are placed in a
foreground layer above the translucent anatomical context, so left- and
right-lateral panels show their complete hemisphere-scoped edge sets instead
of hiding connections behind the mesh.
...
`surface_fsaverage_full.png` through the same paper/pial renderer as
`surface_paper.png`. Library plotting calls still require explicit geometry and
never substitute or download a template silently.
...
Matplotlib edges are 3D lines rather than physical tubes. For auditability they
are intentionally drawn above the anatomical mesh, so their foreground order
must not be interpreted as physical depth. Single-hemisphere panels omit
cross-hemisphere edges; whole-brain panels include them. Plotly uses one trace
per edge and large edge sets produce large HTML files. The first release
focuses on cortical ROIs and does not provide complete mixed-source or
subcortical surface rendering.
